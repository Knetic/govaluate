...
func TestNoParameterEvaluation(test *testing.T) {
	...
	evaluationTests := []EvaluationTest{
		...
		EvaluationTest{
			...
			Input:    "'2014-01-02 14:12:22.120000' > '2014-01-02 12:12:22'",
			Expected: true,
		},
		EvaluationTest{
		...
		EvaluationTest{
			...
			Functions: map[string]ExpressionFunction{
				"passthrough": func(arguments ...interface{}) (interface{}, error) {
					arg1, arg2 := arguments[0].(json.Number), arguments[1].(json.Number)
					val1, _ := arg1.Int64()
					val2, _ := arg2.Int64()
...
func TestStructFunctions(test *testing.T) {
	...
	resVal, _ := result.(float64)
	if (resVal - 4001.0) > 0.0000000001 {
...
func runEvaluationTests(evaluationTests []EvaluationTest, test *testing.T) {
	...
	for _, evaluationTest := range evaluationTests {
		...
		if result != evaluationTest.Expected {
			...
			test.Logf("Test '%s' failed", evaluationTest.Name)
			test.Logf("Evaluation result '%v', type '%v' does not match expected: '%v', type '%v'", result, reflect.TypeOf(result), evaluationTest.Expected, reflect.TypeOf(evaluationTest.Expected))
			test.Fail()
		}
	}
}
